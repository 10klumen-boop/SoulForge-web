...
function runTests() {
  ...
  test("story kill XP stays chapter-linear", () => {
    ...
    assert.strictEqual(farmZoneMineXp({ chapter: 3, side: false }, false), 9);
    assert.strictEqual(farmZoneMineXp({ chapter: 5, side: false }, true), 25);
  });
  ...
  test("high hunting zones grant much more XP than low", () => {
    ...
    assert.ok(mid > low * 3, "mid " + mid + " vs low " + low);
    assert.ok(hi > mid * 5, "hi " + hi + " vs mid " + mid);
    assert.ok(top > hi * 3, "top " + top + " vs hi " + hi);
  });
