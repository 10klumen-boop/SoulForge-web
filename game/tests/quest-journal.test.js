...
function runTests() {
  ...
  test("zoneQuestStepRewardDef grants story XP on early chapters", () => {
    assert.strictEqual(zoneQuestStepRewardDef("banana_mine", 1).xp, 3);
    assert.strictEqual(zoneQuestStepRewardDef("elven_ruins", 2).xp, 4);
    assert.strictEqual(zoneQuestStepRewardDef("orc_barracks", 3).xp, 14);
  });

  test("zoneChapterRewardDef includes XP for ch1–3", () => {
    assert.strictEqual(zoneChapterRewardDef("banana_mine").xp, 3);
    assert.strictEqual(zoneChapterRewardDef("elven_ruins").xp, 4);
    assert.strictEqual(zoneChapterRewardDef("orc_barracks").xp, 14);
  });
