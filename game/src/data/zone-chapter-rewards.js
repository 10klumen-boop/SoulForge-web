// ===== Данные: награды за прохождение главы (босс повержен) =====
// Adena = economyChapterAdena(chapter) из economy-balance.js (P1 калибровка).

const ZONE_CHAPTER_REWARDS = {
  banana_mine: {
    adena: 112_500,
    soul: 28,
    spirit: 0,
    crystals: { D: 3 },
    lines: [
      "Старейшина благодарит за зачистку — остров снова дышит.",
      "Выплата и кристаллы D — заточи клинок и добери силу до следующей главы.",
    ],
  },
  elven_ruins: {
    adena: 225_000,
    soul: 36,
    spirit: 16,
    crystals: { D: 3, C: 1 },
    lines: [
      "Барьер руин удержан — духи отступили от сводов.",
      "Руда и кристаллы — без авто-залива свитками.",
    ],
  },
  orc_barracks: {
    adena: 393_750,
    soul: 48,
    spirit: 28,
    crystals: { D: 2, C: 2 },
    lines: [
      "Граница у леса устояла — орки откатываются.",
      "Племя и церковь делят трофеи за главу.",
    ],
  },
  dark_cavern: {
    adena: 618_750,
    soul: 62,
    spirit: 40,
    crystals: { C: 3 },
    lines: [
      "Скверна приглушена — тьма отступила на шаг.",
      "Кристаллы C — редкая добыча у границы.",
    ],
  },
  dwarven_depths: {
    adena: 900_000,
    soul: 80,
    spirit: 55,
    crystals: { C: 2, B: 1 },
    lines: [
      "Кратер под контролем — гильдии готовы открыть путь к башне.",
      "Финал пролога: осмысленная выплата за главу.",
    ],
  },
};
