...
function renderPassiveIncomePanel(opts) {
  ...
  root.hidden = false;
  const unlocked = typeof passiveWarehouseUnlocked === "function" ? passiveWarehouseUnlocked() : true;
  const rate = unlocked && typeof passiveRatePerSec === "function" ? passiveRatePerSec() : 0;
  const capSec = typeof passiveCapSec === "function" ? passiveCapSec() : 0;
  const pending = unlocked && typeof passivePendingAdena === "function" ? passivePendingAdena() : 0;
  const pendingSec = unlocked && typeof passivePendingSec === "function" ? passivePendingSec() : 0;
  ...
  const status = opts.status
    ...
    : (!unlocked
      ? '<p class="avatar-boost-status avatar-boost-status--warn">Купи склад — иначе оффлайн-adena не копится</p>'
      : "");
    ...
    '<div class="avatar-boost-head">' +
      ...
      '<div class="avatar-boost-titles">' +
        ...
        '<span class="avatar-boost-meta">' +
          (unlocked ? "оффлайн · кап " + formatPassiveDuration(capSec) : "нужен склад") +
        "</span>" +
      "</div>" +
    "</div>" +
    (unlocked
      ...
      : '<p class="avatar-boost-line">Склад 0/' + maxWh + " · накопление выключено</p>") +
    status +
    '<div class="avatar-boost-actions">' +
      ...
      (nextPrice != null
        ? '<button type="button" class="btn btn-primary btn-sm" id="passiveWarehouseBtn">' +
            ...
            (unlocked ? "Склад +2ч · " : "Открыть склад · ") + fmtA(nextPrice) +
          "</button>"
